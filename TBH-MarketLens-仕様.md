...
- `frame_tpl.png` … 名前枠の左角テンプレート。**TBHのUI倍率「2x」で撮った固定ピクセル＝倍率1.0の基準**。検出は`detect_frames`で**UI倍率1x/1.25x/1.5x/2x/3x（解像度で実ピクセルが変わる）に自動追従**＝再キャリブレーション不要。要点：
  ...
  - **セレスティアル枠は専用テンプレ**：明色シアンの名前バーは通常版もエッジ版も相関が閾値に届かず**枠ごと検出落ち**する（実機cap0で確定。落ちると無関係な弱ピークが枠扱いされカーソルから遠い誤枠だけ残る）。`_hi_variant`(テンプレ内側を実測シアンBGR(230,175,15)に塗替え)を3本目としてmax合成。ただし**単色塗りは誤倍率でも当たる**（実機: 誤倍率0.55で0.651／正倍率1.0で0.736）ため、高速パスの倍率即確定は通常/エッジ版`>=_SCALE_STRONG`(0.6)またはセレ版込み`>=_SCALE_STRONG_HI`(0.72)。セレ版だけ強い時は全grid走査で倍率を取り直す。
