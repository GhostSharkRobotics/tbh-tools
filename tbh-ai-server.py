...
- gems / engravings / inscriptions / equipment / uniqueMods: 装備・装飾の実数値（装飾は「色×部位(武器/防具/アクセ)」で効果が変わる）
  ※equipment は現行入手可のみ(装備レベルは 1/5/10/15/20/30/40/50/65/80)。Ver1.00.07(2026-06-02)でSteam負荷対策により Lv25/35/45/55/60/70/75/85/90 とUncommon以上のTypeB装備はゲームから削除→ removedEquipment[](removed:true) に退避。削除レベルの装備は新規入手・合成・取引不可なので推薦しない（所持済みは使用のみ可）。
