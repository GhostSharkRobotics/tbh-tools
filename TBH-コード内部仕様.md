...
## 10. 確定した数値・式（逆アセンブル／データ実値）

### 合成グレード結果（GradeInfoData の重み→%。同グレ9個投入時）
| グレード | −2 | −1 | 同 | +1 | +2 | 固有slot 装/彫/刻 | 錬金G | cubeExp |
|---|---|---|---|---|---|---|---|---|
| コモン | 0 | 0 | 0 | 95.2 | 4.8 | 0 +0/0/0 | 10 | 2 |
| アンコモン | 0 | 0 | 0 | 96.2 | 3.8 | 1 +0/0/0 | 30 | 6 |
| レア | 0 | 0 | 0 | 97.6 | 2.4 | 1 +1/0/0 | 90 | 18 |
| レジェンダリー | 0 | 0 | 0 | 99.0 | 1.0 | 2 +2/0/0 | 270 | 54 |
| イモータル | 0 | 0 | **49.9** | 49.9 | 0.2 | 2 +2/1/0 | 810 | 162 |
| アルカナ | 0 | 0 | **66.6** | 33.3 | 0.2 | 3 +2/1/1 | 2592 | 518 |
| ビヨンド | 0 | 0 | **76.9** | 23.1 | 0.1 | 3 +2/2/1 | 8294 | 1658 |
| セレスティアル | 0 | 0 | **83.3** | 16.7 | 0 | 3 +2/2/2 | 29029 | 5803 |
| ディバイン | 0 | 0 | **90.9** | 9.1 | 0 | 3 +2/2/2 | 101602 | 20311 |
| コズミック | 0 | 0 | 0 | 0 | 0 | 3 +2/2/2 | 355607 | 71089 |

- **降格(−2/−1)は重み0＝実際には起こらない**（enumに存在するだけ）。
- コモン〜レジェはほぼ確定昇格。**イモータル以降は約半数が据え置き**、上ほど昇格しにくい。**コズミックは全0＝合成不可**（EAddCubeResult.CannotSynthesisCosmic と整合）。
- → ツールの「合成グレUP」チャートは GradeInfoData 由来で**正しい**。一方「合成の種類」チャートは SynthesisDropInfoData 由来で**実行時消費が確認できず要疑い**（§5/§本書冒頭）。

### 箱ドロップ率（StageInfoData の per-stage 実値, 120ステージ）
- **白箱 MonsterDropItemRate**: 8/10/15/20/25/30（通常帯）, 80/120/160（特殊ステージ）。序盤8〜15、後半30、特殊は100超＝確定+追加。
- **青箱 BossDropItemRate**: 80/100/150/200/300/400/600/800/**1000**。序盤80〜100、後半は数百〜1000＝確定+複数箱。
- 既存メモの「白8〜16%/青80〜100%」は序盤の範囲。実際はステージで大きく上がる。

### オフライン報酬式（`iun(経過a, OfflineRewardInfoData, StageInfoData)`）
```
報酬 = a × C × [ D + (1 + S/1000) × E ]
```
a=経過（時間係数）, C/D/E=OfflineRewardInfoData の係数(bfoc/bfod/bfoe), S=StageInfoData の値[+0x88]（到達ステージ依存）。到達が深いほど線形で増える。
...
- 各スキル(68クラス)の個別係数・倍率
- 2000.0上限の対象ステ特定、ステ別の base 値（`iir/gxi` per-stat 定数の意味確定）
- 合成 Fail/GreatSuccess（ECubeSynthesisResult）が上記グレ表とどう対応するか（=同グレ=Fail? 等）の確認
- 箱"Rate">100 の正確な解釈（確定+追加抽選の式）

解析基盤: `/tmp/lib.py`（rva2off=セクション別, il2cppセクション delta 0x1600）+ dump.cs のRVA + capstone。rip相対は base==RIP判定。switchはジャンプテーブル(RVA基準)を辿る。数値はObscuredFloat/Intで暗号化される箇所あり。
