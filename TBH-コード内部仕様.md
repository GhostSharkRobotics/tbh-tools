...
### ステータス合算モデル
- **MODTYPE = FLAT(0) / ADDITIVE(1) / MULTIPLICATIVE(2)** … ステ補正は3層で重なる。一般的ARPG式なら `(base + ΣFLAT) × (1 + ΣADDITIVE) × Π(1+MULTIPLICATIVE)`（結合順の数値確認は要追加解析）。
- **MODSOURCE = BASE(0) / ITEM(1) / ATTRIBUTE(2) / PASSIVE(3) / AccountStatus(4) / StatusEffect(5) / BuffSkill(6) / ENVIROUNMENT(7)** … 補正の出所8種。
...
### 抵抗・回避・ブロックの「上限」は固定値でなく可変ステータス
75%固定キャップではなく、`MaxFireResistance/MaxColdResistance/MaxLightningResistance/MaxChaosResistance`(29-32)、`MaxDodgeChance/MaxBlockChance`(18/19)、`MaxElementalBlockChance/MaxElementalDodgeChance`(62/63) という**ステで上限自体を上げ下げできる**。デフォルト値の数値確認は要追加解析。
